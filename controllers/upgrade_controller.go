...
// out-of-sync driver pod to be restarted in place when the running pod spec and the
// desired DaemonSet template spec have the same DRIVER_CONFIG_DIGEST, i.e. the
// install-relevant config is unchanged (e.g. only a helm.sh/chart label changed). If
// either digest is missing, it returns false and the node takes the full upgrade flow.
func (r *UpgradeReconciler) driverPodRestartOnly(running, desired *corev1.PodSpec) (bool, error) {
